...
public class ShadowCaptureRequestBuilderTest {
  ...
  @Test
  @Config(sdk = VERSION_CODES.P)
  public void testAddRemoveTarget() throws Exception {
    Surface s1 = Shadow.newInstanceOf(Surface.class);
    Surface s2 = Shadow.newInstanceOf(Surface.class);
    Surface s3 = Shadow.newInstanceOf(Surface.class);
    Surface s4 = Shadow.newInstanceOf(Surface.class);

    builder.addTarget(s1);
    builder.addTarget(s2);
    builder.addTarget(s3);
    builder.addTarget(s4);
    builder.removeTarget(s2);
    builder.removeTarget(s4);

    assertThat(shadowOf(builder).getSurfaces()).containsExactly(s1, s3);
  }
